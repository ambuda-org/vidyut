...
pub fn bhasya(p: &mut Prakriya) -> Option<()> {
    ...
    if next.has_u_in(&["izWan", "imani~c", "Iyasu~n"]) {
        if bha.has_text_in(&["sTUla", "dUra", "yuvan", "hrasva", "kzipra", "kzudra"]) {
            ...
            // priya -> prezWa, ...
            p.op_term("6.4.157", i, op::text(sub));
