...
fn try_dirgha_adesha_after_num_agama(p: &mut Prakriya) -> Option<()> {
    ...
    if sup.has_text("Am") && has_nuw_agama {
        ...
        if anga.has_antya('n') {
            if anga.ends_with("in") || anga.has_text_in(&["pUzan", "aryaman", "han"]) {
                let sub = al::to_dirgha(anga.upadha()?)?;
...
fn try_add_num_agama_to_anga(p: &mut Prakriya, i_anga: usize) -> Option<()> {
    ...
    if (is_ugit && !anga.is_dhatu()) || is_ac_dhatu {
            ...
            && !anga.is_samasa()
        {
            ...
            // https://ashtadhyayi.com/sutraani/sk446 for AtLopa nisheda w.r.t
            // JHAL-anta and AC-anta lakshana  and hence no numAgama
            if !actual_anga.has_tag(Tag::FlagAtLopa)
                && (actual_anga.has_antya(JHAL) || actual_anga.has_antya(AC))
            {
