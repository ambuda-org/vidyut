...
fn main() {
    ...
    let result =
        rigveda_lipika.transliterate(text_with_accents, Scheme::HarvardKyoto, Scheme::Devanagari);
    println!(
        "   Input:  {} (note: ' = udatta, _ = anudatta)",
        text_with_accents
    );
    ...
    let mut taittiriya_lipika = Lipika::new().with_extension(yajurveda_taittiriya());
