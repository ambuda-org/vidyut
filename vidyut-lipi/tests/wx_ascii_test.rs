...
fn test_wx_ascii_only_output() {
    ...
    assert!(
        wx_result.is_ascii(),
        "WX output should be ASCII-only, got: {}",
        wx_result
    );
    ...
    assert!(
        wx_result.contains('='),
        "WX should use = for udatta, got: {}",
        wx_result
    );
    assert!(
        !wx_result.contains('\u{0951}'),
        "WX should not contain Unicode udatta mark ॑"
    );
    ...
    assert!(
        wx_result2.contains('_'),
        "WX should use _ for anudatta, got: {}",
        wx_result2
    );
    assert!(
        !wx_result2.contains('\u{0952}'),
        "WX should not contain Unicode anudatta mark ॒"
    );
}
