...
fn test_vedic_across_all_indic_scripts() {
    // Test verse with accents (using new ASCII notation)
...
fn test_vedic_across_all_roman_scripts() {
    ...
    for target_scheme in roman_schemes {
            ...
            Scheme::Iast | Scheme::Iso15919 => {
                assert!(
                    result.contains('\u{0951}')
                        || result.contains('\u{0952}')
                        || result.contains('\u{0301}')
                        || result.contains('\u{0300}')
                        || result.contains('\'')
                        || result.contains('_'),
        ...
        // Round-trip test (skip for IAST/ISO due to combining mark complexity)
        if !matches!(target_scheme, Scheme::Iast | Scheme::Iso15919) {
...
fn test_accent_consistency_across_scripts() {
    ...
    for script in all_scripts {
        ...
        // For IAST and ISO schemes that use combining marks
        let combining_udatta = result.matches('\u{0301}').count(); // combining acute
        let combining_anudatta = result.matches('\u{0300}').count(); // combining grave

        let total_udatta = udatta_count + combining_udatta;
        let _total_anudatta = anudatta_count + combining_anudatta;
