...
fn test_taittiriya_with_phonetic_annotations() {
    ...
    // Should handle the (gm) phonetic annotation
    assert!(result.contains("ṁ") || result.contains("\u{0902}"));
...
fn test_atharvaveda_verse() {
    ...
    assert!(result.contains("भद्रम्") || result.contains("भद्रं"), "Result: {}", result);
    assert!(result.contains("\u{0951}")); // udatta
}
