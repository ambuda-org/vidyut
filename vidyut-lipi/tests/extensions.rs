...
fn test_round_trip_accent_preservation() {
    ...
    // Test round-trip for each scheme pair (skip IAST due to combining mark limitations)
    ...
    for (from_scheme, to_scheme) in scheme_pairs {
        for test_case in &test_cases {
            let mut forward_lipika = Lipika::new().with_extension(rigveda_shakala());
            let mut reverse_lipika = Lipika::new().with_extension(rigveda_shakala());
...
fn test_complex_round_trip_preservation() {
    ...
    let rigveda_tests = vec!["agni=mI_le puro=hitam", "ya=jJasya de_vam", "=agnA_ye"];

    let taittiriya_tests = vec!["i#She tvO#rje tvAq", "vA#yavaqH stha", "de#vo vaqH savitA#"];

    // Test Rigveda
    for test_case in rigveda_tests {
        let mut forward = Lipika::new().with_extension(rigveda_shakala());
        let mut reverse = Lipika::new().with_extension(rigveda_shakala());
        ...
        assert_eq!(
            back_to_hk, test_case,
            "Failed Rigveda round-trip HK->Deva->HK for '{}'",
            test_case
        );
    }
    ...
    for test_case in taittiriya_tests {
        let mut forward = Lipika::new().with_extension(yajurveda_taittiriya());
        let mut reverse = Lipika::new().with_extension(yajurveda_taittiriya());
        ...
        assert_eq!(
            back_to_hk, test_case,
            "Failed Taittiriya round-trip HK->Deva->HK for '{}'",
            test_case
        );
    }
}
...
fn test_vedic_fricatives() {
    ...
    assert!(
        result.contains("\u{1cf5}"),
        "Should contain upadhmaniya (ᳵ). Result: {}",
        result
    );
    assert!(
        result.contains("\u{1cf6}"),
        "Should contain jihvamuliya (ᳶ). Result: {}",
        result
    );
    assert!(
        result.contains("\u{0951}"),
        "Should contain udatta marks. Result: {}",
        result
    );
    ...
    assert_eq!(
        back, text_with_fricatives,
        "Round-trip should preserve fricatives and accents"
    );
}
