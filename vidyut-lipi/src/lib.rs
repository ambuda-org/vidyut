...
pub struct Node {
    mnem: String, // Input mnemonic
    #[allow(dead_code)]
    assoc: String, // If a vowel - then the associated sign.
...
impl Node {
	...
	Node {
	    mnem: "".to_string(),
	    assoc: "".to_string(), // For vowels - associated sign. 
    ...
    fn init(m:String, a:String, tv:String,tt:u8,suf:bool,imp:u8) -> Self {
	Node {
	    mnem: m,
	    assoc: a,
	    tok_val: tv,
...
impl TranslationMap {
    ...
    fn default() -> Self {
	TranslationMap {
	    enc_to_tok: HashMap::new(),
	    tok_to_enc: HashMap::new()
	}
    }
    #[allow(dead_code)]
    fn insert_enc_to_tok(&mut self, node:Node) {
	let insstr: String = (node.mnem).to_string();
	...
	if rval.is_some() {
	    (*self.enc_to_tok.get_mut(&sref).unwrap()).suffix = suff;
	}
    }
	...
	while sref.chars().count() > 1 {
	    ...
	    if cnode.is_none() {
		let node = Node::init(sref.clone(), "".to_string(), "".to_string(),
				      TOK_TRANSPARENT_TYPE, true, IMPLICIT_UNKNOWN);
    ...
    pub fn update_string_with_token_with_implicit(&self, intok: String, instr: &mut String,
						  imp: u8,prev:u8) {
...
pub fn generate_map<P: AsRef<Path>>(path: P, transmap: &mut TranslationMap ) -> &mut TranslationMap {
	...
	// for every string in enc insert an entry into the map.
	    ...
	    let mut node: Node = Node::init(mnem,e.assoc.clone(), e.token.clone(),ttype,false,implicit);

	    //println!("Checking {} in {:#?}", e.enc[indx], keys);
	    // check whether this entry is a prefix to existing keys.
	    node.suffix = check_for_prefix(&keys, &e.enc[indx]);
	    ...
	    keys.push(node.mnem.clone());

	    let mut badbad = node.mnem.clone();
...
pub fn transliterate(instr: String, inmap: &TranslationMap,
		     outmap: &TranslationMap, outstr: &mut String) {
...
//	println!("ch: {} indx: {}", curch, sindx);
	...
	if node.is_none() {
		...
		if mnode.tok_val.eq("TOK_ENCODING_ESCAPE")
		    && mnode.tok_type == TOK_ENCODING_ESCAPE
		{
	...
	if node.is_some() && encoding_on {
	    mnode = node.unwrap();
	    outmap.update_string_with_token_with_implicit(mnode.tok_val.clone(),
							  outstr,
							  mnode.implicit,
							  prevch);
	    keystr.clear();
...
#[cfg(test)]
mod tests {
    use super::{TranslationMap,Node,  TOK_UNKNOWN_TYPE, IMPLICIT_UNKNOWN};

    #[test]
    fn test_insert_enc_into_transmap_valid() {
	let mut transmap: TranslationMap = TranslationMap::default();
	let node:Node  = Node::init("a".to_string(), "".to_string(), "TOK_TEST_TYPE".to_string(),
					TOK_UNKNOWN_TYPE, false, IMPLICIT_UNKNOWN);
